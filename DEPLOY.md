...
Now in Discord try `/assign Notch S`. You should get a pink confirmation. Then `/tier Notch` for
the embed. If `/assign` says you lack permission, your ID is wrong or you didn't restart.
...
all working. Then in Discord, `/assign <someone> A` and re-run:
...
| Command | Does |
|---|---|
| `/assign <player> <tier>` | Assigns a tier. Pick **None** to take them off the list. |
| `/retire <player>` | Toggles retirement: once retires, again brings them back. |
...
| `/assign` says "You do not have permission" | Your ID isn't in `graders.json`, or you added it but didn't restart. Copy **User ID**, not username. |
