 ...
 *
 * <p>{@code updateRenderState} is not guaranteed to run once per state per
...
public final class Tagged {
	private static final Map<EntityRenderState, Text> TAGGED = new WeakHashMap<>();

	private Tagged() {
	}

	/** True when {@code name} is one this class already produced for {@code state}. */
	public static boolean isTagged(EntityRenderState state, Text name) {
		Text previous = TAGGED.get(state);
		return previous != null && previous.equals(name);
	}

	public static void remember(EntityRenderState state, Text name) {
