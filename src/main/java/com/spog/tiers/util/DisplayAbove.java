 ...
 *
 * <p>Outside {@code com.spog.tiers.mixin} because everything in that package
...
public final class DisplayAbove {
	private static final Map<TextDisplayEntityRenderState, Text> LABELS = new WeakHashMap<>();

	private DisplayAbove() {
	}

	public static void set(TextDisplayEntityRenderState state, Text label) {
		if (label == null) {
			LABELS.remove(state);
		} else {
	...
	public static Text get(TextDisplayEntityRenderState state) {
