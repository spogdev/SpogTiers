...
public class ProfileScreen extends Screen {
	...
	private void drawHeader(DrawContext graphics) {
		...
		nameCopyRight = left + Math.round((nameX + textRenderer.getWidth(playerName)) * scale);
		nameCopyBottom = top + Math.round((nameY + textRenderer.fontHeight) * scale);
		...
		int tagX = nameX + textRenderer.getWidth(playerName) + 5;
	 ...
	 */
	private void drawDiscord(DrawContext graphics, int x, int y) {
		...
		// them on no evidence.
		graphics.drawTextWithShadow(font, Text.literal(account.labels()), cursor, y,
		...
		discordCopyRight = MARGIN + Math.round((cursor + textRenderer.getWidth(account.labels())) * scale);
		discordCopyBottom = MARGIN + Math.round((y + textRenderer.fontHeight) * scale);
	}
	 ...
	 */
	@Override
	public boolean mouseClicked(net.minecraft.client.gui.Click event,
			boolean doubled) {
	...
	private int drawGradeTag(DrawContext graphics, int x, int y, PlayerGrade grade) {
		...
		graphics.drawTextWithShadow(textRenderer, icon, x + 3, y + 2, 0xFFFFFFFF);
