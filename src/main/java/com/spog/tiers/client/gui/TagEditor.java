...
public final class TagEditor {
	...
	private void codeField(int x, int y, int width) {
		if (codeField == null || codeField.getWidth() != width) {
			...
			// instead.
			codeField = new EditBox(font, x, y, width, font.lineHeight,
			...
			codeField.setBordered(false);
			codeField.setTextColor(TEXT_COLOR);
			codeField.setHint(Component.literal("Paste a code"));
	 ...
	 */
	private void drawCodeField(GuiGraphicsExtractor graphics, int x, int y, int width,
			int mouseX, int mouseY) {
		...
		// top edge.
		int textY = y + (BUTTON_HEIGHT - font.lineHeight) / 2;
	 ...
	 */
	private void drawCodeRow(GuiGraphicsExtractor graphics, int left, int y, int right,
			int mouseX, int mouseY) {
		...
		if (complaint != null) {
			graphics.text(font, Component.literal(complaint), left + PADDING,
					y - font.lineHeight - 3, complaintGood ? GOOD_FILL : OFF_FILL);
		}
	...
	private void drawElementText(GuiGraphicsExtractor graphics, TagLayout.Element element,
		...
		if (element.kind == TagLayout.Kind.DISCORD) {
			...
			Component icon = SpogTiersClient.config().showTagIcons
	...
	public boolean click(net.minecraft.client.input.MouseButtonEvent event, boolean doubled) {
	...
	private static boolean inside(EditBox box, double x, double y) {
	...
	private int span(TagLayout.Element element) {
		...
		if (element.kind == TagLayout.Kind.DISCORD) {
			int width = font.width(discordPreview());
			if (SpogTiersClient.config().showTagIcons) {
				Component icon = ModeIcons.discord();
				width += font.width(icon) + font.width(" ");
			}
			return width + 6;
		}
