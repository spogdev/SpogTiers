...
public final class TagEditor {
	...
	private void codeField(int x, int y, int width) {
		if (codeField == null || codeField.getWidth() != width) {
			...
			codeField = new EditBox(font, x, y, width, CODE_HEIGHT,
	...
	private void drawCodeField(GuiGraphicsExtractor graphics, int x, int y, int width,
		...
		// top edge.
		int textY = y + (CODE_HEIGHT - font.lineHeight) / 2;
	...
	private void drawPreview(GuiGraphicsExtractor graphics, int left, int top, int right,
		...
		if (complaint != null) {
			graphics.text(font, Component.literal(complaint), left + PADDING, y,
