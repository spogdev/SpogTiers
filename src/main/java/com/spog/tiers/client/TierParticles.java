...
public final class TierParticles {
	...
	public static void tick(MinecraftClient client) {
		...
		List<? extends AbstractClientPlayerEntity> players = client.world.getPlayers();
		for (AbstractClientPlayerEntity player : players) {
			if (player.isInvisible() || player.isSpectator()) {
				continue;
			}
			if (player.squaredDistanceTo(camera) > RANGE_SQ) {
				continue;
			}
	...
	private static void emit(MinecraftClient client, AbstractClientPlayerEntity player, int rgb) {
		var random = player.getRandom();
		double height = player.getHeight();
		float red = ((rgb >> 16) & 0xFF) / 255.0f;
		float green = ((rgb >> 8) & 0xFF) / 255.0f;
		float blue = (rgb & 0xFF) / 255.0f;
		TintedParticleEffect option =
		...
		for (int i = 0; i < PER_EMIT; i++) {
			...
			client.world.addParticleClient(option, x, y, z,
