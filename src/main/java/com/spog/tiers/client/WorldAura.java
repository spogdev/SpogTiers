...
public final class WorldAura {
	...
	public WorldAura() {
		...
		for (int i = 0; i < MOTES; i++) {
			...
			angle[i] = (i * MathHelper.TAU / MOTES) + (random.nextFloat() - 0.5f) * 0.6f;
	...
	public float flicker(int mote, float elapsed) {
		float wave = MathHelper.sin(elapsed * flickerRate[mote] + wobble[mote]);
	...
	public float theta(int mote, float elapsed) {
		...
		float jitter = MathHelper.sin(elapsed * 7.0f * speed[mote] + wobble[mote] * 3.0f) * JITTER;
	...
	public float radius(int mote, float elapsed) {
		float breath = MathHelper.sin(elapsed * 0.8f * speed[mote] + wobble[mote] * 2.0f) * BREATH;
	...
	public float across(int mote, float elapsed) {
		return 0.5f + 0.5f * radius(mote, elapsed) * MathHelper.cos(theta(mote, elapsed));
	}

	/** Position through the body, -1 at the back of the aura to 1 at the front. */
	public float depth(int mote, float elapsed) {
		return radius(mote, elapsed) * MathHelper.sin(theta(mote, elapsed));
	}
	...
	private static int mix(int from, int to, float amount) {
		return ColorHelper.getArgb(
				MathHelper.lerp(amount, ColorHelper.getRed(from), ColorHelper.getRed(to)),
				MathHelper.lerp(amount, ColorHelper.getGreen(from), ColorHelper.getGreen(to)),
				MathHelper.lerp(amount, ColorHelper.getBlue(from), ColorHelper.getBlue(to))) & 0xFFFFFF;
	}

	private static int withAlpha(int rgb, float alpha) {
		return (MathHelper.clamp((int) (alpha * 255.0f), 0, 255) << 24) | (rgb & 0xFFFFFF);
	}
}
