...
public final class TagEditor {
	...
	private void drawGeneral(DrawContext graphics, int left, int top, int right,
		...
		y = rule(graphics, left, right, y);
		graphics.drawTextWithShadow(font, Text.literal("Icons"), x, y, LABEL_COLOR);
		y += font.fontHeight + 6;
	 ...
	 */
	private void drawElementFooter(DrawContext graphics, int left, int right,
			int bottom, int mouseX, int mouseY) {
		int footer = bottom - FOOTER_HEIGHT;
	 ...
	 */
	private void middle(DrawContext graphics, String message, int left, int top,
			int right, int bottom) {
		List<net.minecraft.text.OrderedText> lines =
				font.wrapLines(Text.literal(message), right - left - PADDING * 2);
		int y = top + (bottom - top - lines.size() * font.fontHeight) / 2;
		for (var line : lines) {
			graphics.drawTextWithShadow(font, line,
					left + (right - left - font.getWidth(line)) / 2, y, MUTED_COLOR);
			y += font.fontHeight;
		}
	}
	...
	private Text icon(TagLayout.Element element) {
		...
		Text glyph = GlyphIcons.of(SpogTiersClient.config().iconStyle, mode);
