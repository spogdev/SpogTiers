...
import dev.spog.tiers.client.SkinTextures;
...
import dev.spog.tiers.data.TierDetail;
...
public class ProfileScreen extends Screen {
	...
	protected void init() {
		...
		AnimatedSkinWidget skinWidget = new AnimatedSkinWidget(
				skinWidgetWidth(), skinHeight, client.getEntityModels(), this::modelSkin);
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
		...
		for (TileBounds tile : tiles) {
			...
			PlayerSkin picked = SkinTextures.ready(tile.entry());
			if (picked == null) {
				return false;
			}
	...
	private List<SkinHistory.Entry> tileEntries() {
		...
		for (SkinHistory.Entry entry : history.entries()) {
			...
			SkinTextures.get(entry);
			if (SkinTextures.ready(entry) != null) {
				ready.add(entry);
			}
		}
		return ready;
	 ...
	 */
	private void drawSkinTiles(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		tiles.clear();
		if (skinWidget == null || exporting) {
			return;
		}
		...
		for (int i = 0; i < shown; i++) {
			...
			PlayerSkin loaded = SkinTextures.ready(entry);
            if (loaded == null) {
				continue;
			}
	...
	private void drawTileMenu(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		TileMenu menu = tileMenu;
		...
		for (int i = 0; i < menu.labels().size(); i++) {
			...
			graphics.text(font, Component.literal(menu.labels().get(i)),
					menu.left() + MENU_PADDING * 2,
					rowTop + (MENU_ROW_HEIGHT - font.lineHeight) / 2 + 1,
					LABEL_COLOR);
		}
	}
	...
	private void runTileMenu(TileMenu menu, int row) {
		...
		Minecraft client = Minecraft.getInstance();
		CompletableFuture
				.supplyAsync(() -> saveSkin(entry), Util.ioPool())
	...
	private void drawTileFace(GuiGraphicsExtractor graphics, PlayerSkin worn, int x, int y) {
		graphics.blit(RenderPipelines.GUI_TEXTURED, worn.body().texturePath(),
